...
spec = do
  describe "TodoValidation List" $ do
    it "should create a todo item" $ do
       Todo { description ="something", state = Active } `shouldBe` Todo { description ="something", state = Active }
    ...
    it "should fail validation with multiple errors" $ do
       addValidatedTodo defaultValidations Todo { description =" ", state = Complete }  [] `shouldBe` (Left $ InvalidDescriptionError :| [InvalidStatusError])
    ...
    it "given a todo then mark it as complete" $ do
       Todo { description ="something", state = Complete } `shouldBe` (complete Todo { description ="something", state = Active })
    ...
    where
        todoEmptyDescription = Todo { description =" ", state = Active }
        todoActive = Todo { description ="todo marked as active", state = Active }
        todoComplete = Todo { description ="todo marked as complete", state = Complete }
        todoList = [todoActive, todoComplete]
        expectedUpdatedTodoList = [Todo { description ="todo marked as active", state = Complete }, Todo { description ="todo marked as complete", state = Complete }]
